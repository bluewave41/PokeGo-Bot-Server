...
module.exports = {
    ...
    async addXP(userId, xp) {
        ...
        console.log(newXP, requiredXPForLevel)
    ...
    async getUserInfo(userId) {
        const user = await User.query().select('xp', 'level', 'currency', 'totalxp', 'stardust', 'storage', 'itemstorage')
            .where('userId', userId);
        return user[0];
    },
    ...
    async getSaved(userId) {
        const user = await User.query().select('saved')
            .where('userId', userId);
        return JSON.parse(user[0].saved);
    },
