module.exports = {
    ...
    getError(message, errorGroup, replace) {
        const replacements = [].concat(replace || []);
        let errorMessage = this[errorGroup]['en'][message];
        if(!errorMessage) {
            return `I got ${message} but it wasn't in the list.`;
        }
